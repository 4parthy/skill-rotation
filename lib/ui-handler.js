...
module.exports = function UIHandler(mod) {
    ...
    this.display = (rotation, history, inCombat, force = false) => {
        ...
        if (rotation.length > 1) {
            msg += ` <font color="#FFFFFF" size="18">Future: ${rotation.slice(1, 4).map(s => s.name).join(' > ')}</font>`;
        }
        ...
        const now = Date.now();
        if (!force && msg === this.lastMsg && now - this.lastTime < this.minInterval) {
            return;
        }
        ...
        mod.log(`Displaying rotation: Next=${nextSkill.name}`);
